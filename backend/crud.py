...
def get_messages_in_chat(db: Session, owner_id: str, skip: int = 0, limit: int = 100):
    return db.query(models.Message).filter(models.Message.chat_id == owner_id).offset(skip).limit(limit).all()


def create_message(message: str, chat_id: str, db: Session):
    print(chat_id)
    myuuid = uuid.uuid4()
    db_message = models.Message(id=myuuid, message=message, chat_id=chat_id, typeOfMessage="text")
    print(db)
    print(db_message)
    db.add(db_message)
    db.commit()
